...
export class ParticleSystem {
  ...
  trail(position, color) {
    // Lightweight trail particle each frame
    if (this.particles.length >= this.max) return;
    const vel = new THREE.Vector3((Math.random() - 0.5) * 0.6, (Math.random()) * 0.4, (Math.random() - 0.5) * 0.6);
    const life = 0.35 + Math.random() * 0.25;
    this.particles.push({ pos: position.clone(), vel, life, age: 0, color });
  }
